...
float AngleEva(const Stage& aStage) {
	float sumAngle = 0;
	float minDist = 1000;
	float sumDist = 0;
	int Target = 0;
	...
	for (int j = 0; j < AsteroidCnt; j++) {
		if (aStage.asteroid(j).exists() && !targetAsteroid[j]) {
			float dist = aStage.ship().pos().dist(aStage.asteroid(j).pos());
			if (dist < minDist) {
				minDist = dist;
				Target = j;
			}
	...
	for (int i = 0; i < AsteroidCnt; i++) {
		if (aStage.asteroid(i).exists() && targetAsteroid[i]) {
			if (flag) {
				dir0 = aStage.asteroid(i).pos() - aStage.asteroid(Target).pos();
				flag = false;
			}
			Vector2 dir = aStage.asteroid(i).pos() - aStage.asteroid(Target).pos();
			sumAngle += abs(dir0.rotSign(dir)) * (dir - dir0).length();
		}
	}
	...
	for (int i = 0; i < AsteroidCnt; i++) {
		if (aStage.asteroid(i).exists() && targetAsteroid[i]) {
			Vector2 dir = aStage.asteroid(i).pos() - aStage.asteroid(Target).pos();
			res += (dir - dir0).length()*abs(dir0.rotSign(dir))/sumAngle;
		}
	}
...
Action Answer::getNextAction(const Stage& aStage)
{
	...
	if (aStage.ship().canShoot()) {
		...
		for (int j = 0; j < 360; j++) {
			...
			x.push_back(make_pair(DistEva(aStage,Vector2::Zero()) + AngleEva(aStage) + MinDist(aStage,Vector2::Zero()), j));
		...
		if (update) {
			...
			for (int i = 0; i < 360; i++) {
				...
				for (int j = 0; j < 360; j++) {
					...
					if (flag && cnt)x.push_back(make_pair((ship.pos()+moveVector).dist(aStage.asteroid(target).pos())+AngleEva(aStage)+DistEva(aStage,moveVector) + MinDist(aStage, moveVector), j));
					laser.rotateRad(rad1);
				}
				sort(x.begin(), x.end());
