...
public:
		...
		while (1) {
			...
			while (1) {
				...
				for (int t = 0; t < SPLIT*SPLIT; t++) {
					pi = make_pair(section.first.first + (section.second / SPLIT) * (t / SPLIT) + rx.rand() % (section.second / SPLIT), section.first.second + (section.second / SPLIT) * (t % SPLIT) + rx.rand() % (section.second / SPLIT));
					if (G[pi.first][pi.second])continue;
	...
	bool Chance2() {
		...
		else if (t > 0.6)probability = 0.05;
		else if (t > 0.4)probability = 0.2;
		else if (t > 0.2)probability = 0.5;
