...
public:
	...
	pair<int,vector<vector<int>>> ChainCnt(vector<vector<int> >nextBlock, vector<pair<int, int> >pos, int chainCnt) {
		bool HasChain = false;
		...
		if (!have_Chain) return make_pair(chainCnt-1,nextBlock);

		chainCnt = ChainCnt(nextBlock, moved_blocks, chainCnt + 1).first;
		return make_pair(chainCnt,nextBlock);
	}
...
public:
	...
	int chainMode(vector<vector<int> >field,int turn_num,int cnt) {
		...
		for (int i = 0; i < 4; i++) {
			...
			for (int j = 0; j < W - packWidth + 1; j++) {
				...
				//評価
				pair<int, vector<vector<int>>> chainCnt = myField.ChainCnt(next_field, next_packPos, 1);
				if (!isDanger && chainCnt.first==0) {
					int now_rotateCnt = packs[turn].rotateCnt;
					num = chainMode(chainCnt.second, turn_num + 1, cnt + 1);
					chain.push_back(num);
					packs[turn].rotate((now_rotateCnt + 4 - packs[turn].rotateCnt) % 4);
				}
				else if (!isDanger) {
					chain.push_back(chainCnt.first);
				}
	...
	struct CHAIN {
		int num, pos, rot, side_first;

		bool operator<(const CHAIN& right)const {
			return num == right.num ? pos<right.pos : num> right.num;
		}
	};
	...
	void executeTurn() {
		...
		for (int i = 0; i < 4; i++) {
			...
			for (int j = 0; j < W - packWidth + 1; j++) {
				...
				//評価
				pair<int,vector<vector<int>>> chainCnt = myField.ChainCnt(next_field, next_packPos,1);
				if (!isDanger && chainCnt.first >= burstNumber) {
					chainNumber = 1;
					rot = i;
					pos = j;
					chain.push_back({ chainCnt.first,pos, rot, sides.first });
				}
				else if (!isDanger&&chainCnt.first==0) {
					int now_rotateCnt = packs[turn].rotateCnt;
					int flag = chainMode(chainCnt.second,turn + 1,1);
					packs[turn].rotate((now_rotateCnt + 4 - packs[turn].rotateCnt) % 4);
					if (flag>chainNumber) {
						chainNumber = flag;
						rot = i;
						pos = j;
						chain.push_back({ flag,pos, rot, sides.first });
					}
					else {
						chain0Pos = j;
						chain0Ang = i;
						chain0.push_back({ 0,chain0Pos, chain0Ang, sides.first });
					}
				}
		...
		if (pos == -3) {
			if (chain0Pos != W) {
				...
				pos = chain0[0].pos - chain0[0].side_first;
				rot = chain0[0].rot;
			}
			else{
				//pos = insertPos;
