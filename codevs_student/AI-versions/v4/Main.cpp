...
public:
	...
	int ChainCnt(vector<vector<int> >nextBlock, vector<pair<int, int> >pos, int chainCnt) {
				...
				while (1) {
					...
					if (sum > 10)break;
						...
						for (int l = 0; l < cnt + 1; l++) {
							if (!HasChain)for (int m = 0; m < pos.size(); m++) {
								//間に挟む時が見れていない
								if (pos[i].first + k*l == pos[m].first&&pos[i].second + j*l == pos[m].second) break;
								if (m == pos.size() - 1)HasChain = true;
							}
		...
		if (!have_Chain) return chainCnt-1;
	...
	void executeTurn() {
		...
		for (int i = 0; i < 4; i++) {
			...
			for (int j = 0; j < W - packWidth + 1; j++) {
				...
				//評価
				int chainCnt = myField.ChainCnt(next_field, next_packPos,1);
		...
		if (pos == -3) {
			if (chain0Pos != W) {
				...
				for (int i = 0; i < 4; i++) {
					...
					pos = myField.min_point(packWidth, packHeights)-sides.first;
					rot = i;
					packs[turn].rotate(1);
				}
			}
